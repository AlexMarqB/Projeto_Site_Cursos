...
import { Modules } from "@/@example_arrays";
...
export default function AdminModules() {
   ...
   const selectedData = Modules.filter(item => item.name.toLowerCase().includes(search.toLocaleLowerCase())).slice(inicio, fim);

   return (
      <>
         <div className="flex justify-between">
            <h1 className="text-zinc-50 text-3xl font-medium">Modules</h1>
            <Bnt.PurpleButton onClick={() => navigate('/admin/modules/enroll')}>Cadastrar</Bnt.PurpleButton>
         </div>
         ...
         <Divider />
            <ResponsiveTableOptions>
               ...
               <h1 className="text-3xl font-bold text-zinc-100 self-center">{actualPage}</h1>
               </div>
               </div>
            </ResponsiveTableOptions>
            <Tbl.Root>
                  ...
                  <Tbl.TBody>
                     {selectedData.map((item, index) => (
                        <Tbl.TRow key={index}>
                           ...
                           <Tbl.TDesc>R$ {item.description}</Tbl.TDesc>
                           <Tbl.TDesc>{item.teacherId}</Tbl.TDesc>
                           <Tbl.TDesc>{item.courseId}</Tbl.TDesc>
                           <Tbl.TDesc>
